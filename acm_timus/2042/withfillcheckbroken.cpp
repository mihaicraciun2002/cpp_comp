...
void checkLastFill(const int& node, const int& treeLeft, const int& treeRight, const int& segmentNode)  {
        ...
        if(1)  {
            if(arbint[node].lastFill > arbint[segmentNode].lastFill)  {
                arbint[segmentNode].filled = true;
                arbint[segmentNode].fillCh = arbint[node].fillCh;
                arbint[segmentNode].calculated = false;
...
/*     if(arbint[node].filled)  {
    ...
    if(treeMiddle + 1 <= l)  {
        if(addCalculated)
            arbint[node].ans = pieceWise, arbint[node].calculated = true;
        return pieceWise;
    }
    if(treeMiddle >= r)  {
        if(addCalculated)
...
/*     checkLastFill(1, 1, n, sons[0]);
    checkLastFill(1, 1, n, sons[1]); */
    if(0 && arbint[sons[0]].filled && arbint[sons[1]].filled)  {
        if(arbint[sons[0]].fillCh == arbint[sons[1]].fillCh)  {
            for(int partial1 = 1;partial1 <= std::min(k - 1, length1);partial1++)  {
                ...
                long long sumStop = std::min(length2, k - partial1);
                pieceWise += sumStop;
            }
        }
    }else{
